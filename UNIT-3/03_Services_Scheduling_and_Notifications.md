...
```java
// 1. Create custom JobService
public class MyNotificationJobService extends JobService {

    @Override
    public boolean onStartJob(JobParameters params) {
        // Perform background work on worker thread
        new Thread(() -> {
            // Task logic here
            jobFinished(params, false); // Signal job completion
        }).start();
        return true; // Return true if task offloaded to thread
    }

    @Override
    public boolean onStopJob(JobParameters params) {
        return true; // Return true to reschedule if job cancelled
    }
}
